...
int main()
{
    ...
    do  // Anzeige des Men�s
    {
        ...
        switch (abfrage)
        {
            ...
            case '7':
            {
                std::cout << "Das Menu wird nun beendet." << std::endl;
                break;
            }
...
void mediumEntfernen(std::vector<Medium*>& medien)
{
    ...
    for(std::vector<Medium*>::iterator it = medien.begin(); it != medien.end(); it++)
    {
        if((*it)->getID() == id)
        {
            if ((*it)->get_status())
            {
                std::cout << "Dieses Medium ist ausgeliehen und kann nicht entfernt werden" << std::endl;
                success = true; 
                break;
            }
            else
            {
                delete (*it);  // Objekt loeschen
                medien.erase(it); // Zeiger auf das Objekt l�schen
                success = true; // Loeschen war erfolgreich
...
void mediumAusleihen(std::vector<Medium*>& medien, Datum aktuellesDatum)
{
    ...
    if (!id_vorhanden)
    {
        std::cout << "Keine gultige ID!" << std::endl;
    }
}
